...
use std::simd::{mask64x8, u64x8, u32x8};
...
impl U128x8 {
    ...
    pub fn is_zero(self) -> mask64x8 {
        (self.hi.cast() | self.lo).lanes_eq(u64x8::splat(0))
    }

    #[inline]
    pub const fn splat(x: u128) -> Self {
        Self {
            hi: u32x8::splat((x >> 60) as u32),
    ...
    pub fn sub_on_ge(self, other: Self) -> Self {
        ...
        let hi_eq = c_hi.lanes_eq(u32x8::default());
        let hi_ge = (c_hi & u32x8::splat(1 << 31)).lanes_eq(u32x8::default());
        let lo_ge = (c_lo >> u64x8::splat(63)).lanes_eq(u64x8::default());
        let ge = hi_eq.select_mask(lo_ge.into(), hi_ge);
        c_hi = c_hi - (c_lo >> u64x8::splat(63)).cast();
        c_lo = c_lo & u64x8::splat(MASK);
        Self {
            hi: ge.select(c_hi, self.hi),
            lo: mask64x8::from(ge).select(c_lo, self.lo),
        }
    }
    ...
    pub fn mul10_add(self, b: u64) -> Self {
        ...
        let hi = self.hi * u32x8::splat(10) + (lo >> u64x8::splat(60)).cast();
        Self {
            hi,
